...
public class UsuarioController {
    @Autowired
    private UsuarioService apicepService;

    @PostMapping
    public UsuarioModel salvar(@RequestBody UsuarioModel apicepModel) {
        return apicepService.salvar(apicepModel);
    }

    @PostMapping("/login")
    public ResponseEntity<?> login(@RequestBody LoginRequest loginRequest) {
        Optional<UsuarioModel> usuario = apicepService.buscarPorEmail(loginRequest.getEmail());
    ...
    @PutMapping("/{id}")
    public ResponseEntity<UsuarioModel> update(@PathVariable Long id, @RequestBody UsuarioModel apicepModel) {
        if (apicepService.findByID(id).isPresent()) {
